...
public class FileUploadServiceImpl implements FileUploadService {
    ...
    public String uploadFile(MultipartFile file) {
        ...
        String fileNameWithExtension = UUID.randomUUID().toString() + "." + fileNameExtension;
        ...
        try{
            ...
            if(response.sdkHttpResponse().isSuccessful()){
                return "https://" + bucketName + ".s3.amazonaws.com/" + fileNameWithExtension;
            }else{
                throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "An error occured while uploading the image");
            }

        }catch (IOException e){
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "An error occured while uploading the file to aws S3 bucket");
        }
    }
